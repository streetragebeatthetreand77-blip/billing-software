  ...
  const handlePrintBarcode = (p: Product) => {
    ...
    printEl.innerHTML = `
      <div style="font-family: monospace; text-align: center; font-size: 10px; width: 50mm; height: 25mm; display: flex; flex-direction: column; align-items: center; justify-content: center; background: white; color: black; box-sizing: border-box; padding: 2px;">
        <div style="font-size: 9px; font-weight: bold; letter-spacing: 0.5px; text-transform: uppercase;">STREET RAGE</div>
        <div style="font-size: 8px; margin: 1px 0; color: #666666; font-weight: 500; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; width: 44mm;">${p.name}</div>
        <div class="font-barcode" style="font-size: 42px; margin: 1px 0; line-height: 1; font-family: 'Libre Barcode 128', sans-serif;">*${p.id}*</div>
        <div style="font-size: 8px; letter-spacing: 1.5px; font-family: monospace; font-weight: 500;">${p.id}</div>
        <div style="font-size: 9px; font-weight: bold; margin-top: 1px;">₹${p.price.toLocaleString('en-IN')}</div>
      </div>
    `;

    const styleEl = document.createElement('style');
    styleEl.innerHTML = `@page { size: 50mm 25mm; margin: 0; }`;
